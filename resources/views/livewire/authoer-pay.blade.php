<div>
    ...
    <button type="button"
        ...
        x-data x-on:click="$dispatch('open-modal',{ name : 'add-pay-modal'})">اضافة   </button>
    <br>
    <hr>
                            ...
                            <tr class="gridjs-tr">
                                ...
                                <td class="gridjs-td"><span>
                                        ...
                                        type="button" wire:click='setname({{ $p->id }})' x-data
                                                x-on:click="$dispatch('open-modal',{name:'edit-pay-modal'})"
                                        class="w-8 h-8 p-0 btn text-info hover:bg-info/20 focus:bg-info/20 active:bg-info/25">
                                            ...
                                            x-data
                                            x-on:click="$dispatch('open-modal',{name:'del-pay-modal'})"
    ...
    <div wire:ignore.self>
        <x-modaladd  title="اضافة مبالغ اخرى  " name="add-pay-modal">
            @slot('body')
            <form>
                <div class="p-2 space-y-6">
                    <label class="block">
                        <span>   نوع المبلغ  </span>
                        <input wire:model='name'
                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                            placeholder="  ادخل نوع المبلغ هنا ...  " type="text" />
                    </label>
                    @error('name')
                ...
                <div class="items-center p-4 border-gray-200 rounded-b dark:border-gray-700">
                    ...
                    <button type="button" wire:click.prevent='storePayData'
    ...
    <div wire:ignor.self>
        <x-modaladd title="تعديل بيانات انواع المبالغ الاخرى" name="edit-pay-modal">
            <x-slot:body>
                <form>
                    <div class="p-2 space-y-6">
                        ...
                        <label class="block">
                            ...
                            <input wire:model='ed_name'
                                class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                placeholder="  ادخل نوع المبلع هنا ...  " type="text" />
                        </label>
                        @error('ed_name')
                        <span class="text-tiny+ text-error">
                            {{ $message }}
                    ...
                    <div class="items-center p-4 border-gray-200 rounded-b dark:border-gray-700">
                        ...
                        <button type="button" wire:click.prevent='editPayData'
                            class="font-medium text-white btn bg-primary hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
                            تأكيد
                        </button>
                    </div>
    ...
    <x-modaldel wire:ignore.self name="del-pay-modal">
        @slot('delbody')
        <div class="mt-4">
            ...
            <p class="mt-2">
                هل انت متأكد من انك تريد حذف هذا الحي
            </p>
            <button wire:click="deletePayData"
